...
from ast import Mod
import os
from flask import Blueprint,render_template,request,send_file,session,redirect,flash,url_for
          ...
          if Mode=="Cooling only":
            ...
            if Steam=="Steam_No":
                ...
                if Heater=="Dehumidifiction(Reheat)" :
                  ...
                  with open(Config.all_sensors+sensors+".txt","r") as k:
                      t=k.read()
...
################ HP##########################
    ...
    ############# protections##########
    with open(Config.all_protections+"STD"+".txt","r") as q8:
                      t8=q8.read()
    ...
    if protections=="STD":
      ...
      with open(Config.all_protections+protections+".txt","r") as q9:
                      t9=q9.read()
                      protections_list.append(t9)
                      
    elif protections=="CB FOR COMP+CFM":
      with open(Config.all_protections+protections+".txt","r") as q1:
                      t=q1.read()
    ...
    with open(Config.all_modes+coolig_mode+".txt","r") as q10:
                      t10=q10.read()
    ...
    if Mode=="Heat pump":
      ...
      with open(Config.all_modes+heatpump_mode+".txt","r") as q10a:
                      t10a=q10a.read()
                      heatpump_modes_list.append(t10a)
    
      defrost_mode="DEFROST"

      with open(Config.all_modes+defrost_mode+".txt","r") as q10b:
                      t10b=q10b.read()
    ...
    if Heater=="Heating":
      ...
      with open(Config.all_modes+heating_mode+".txt","r") as q10:
                      t10=q10.read()
      ...
      with open(Config.all_modes+heating_mode1+".txt","r") as q10:
                      t10=q10.read()
                      heat1_modes_list.append(t10)
      
      heating_mode2="HEATER FOR DEHUM"
      with open(Config.all_modes+heating_mode2+".txt","r") as q11:
                      t11=q11.read()
    ...
    if Dampers=="Mixing":
      ...
      with open(Config.all_options+Dampers_mode+".txt","r") as q30:
                      t30=q30.read()
    ...
    if special_sensors:
      ...
      with open(Config.sensors,"w") as k1:
    ...
    with open(Config.modes+"defrost_mode.txt","w") as k8:
    ...
    if Controller=='Carel':
      pdf.image(Config.all_images+"PCO_LOGO.png",x=40,w=130,h=80)
    else:
      pdf.image(Config.all_images+"POL.png",x=40,w=130,h=80)
    ...
    pdf.set_font('Arial', '', 10)
    f5 = open(Config.all_txt+"STARTUP.txt", "r")
    for x13 in f5:
    ...
    pdf.set_font('Arial', '', 10)
    f5 = open(Config.all_txt+"SYSTEMSTART.txt", "r")
    for x13 in f5:
      pdf.cell(0, 10,txt = x13, ln = 1, align = 'L')
    ...
    pdf.set_font('Arial', '', 10)
    f6 = open(Config.modes+"cooling_mode.txt", "r")
    for x14 in f6:
      pdf.cell(0, 10,txt = x14, ln = 1, align = 'L')
    pdf.image(Config.all_images+"COMPRESSORS_FUNCTION.png",x=50,w=80)

    if Mode=="Cooling only" and Heater=="Heating":
      pdf.set_font('Arial', 'B', 10)
      pdf.cell(0,8,txt='Heating Mode:',ln=1,align='L')
      pdf.set_font('Arial', '', 10)
      f7 = open(Config.modes+"heating_mode.txt", "r")
      for x15 in f7:
      ...
      pdf.cell(0,8,txt='Heating Mode:',ln=1,align='L')
      pdf.set_font('Arial', '', 10)
      f7 = open(Config.modes+"heating_mode1.txt", "r")
      for x15 in f7:
          pdf.cell(0, 10,txt = x15, ln = 1, align = 'L')
      ...
      pdf.cell(0,8,txt='Dehumidifiction Mode:',ln=1,align='L')
      pdf.set_font('Arial', '', 10)
      f8 = open(Config.modes+"heating_mode2.txt", "r")
      for x16 in f8:
      ...
      pdf.set_font('Arial', '', 10)
      f7 = open(Config.modes+"heatpump_mode.txt", "r")
      for x15 in f7:
          pdf.cell(0, 10,txt = x15, ln = 1, align = 'L')
      ...
      pdf.set_font('Arial', '', 10)
      f8 = open(Config.modes+"defrost_mode.txt", "r")
      for x16 in f8:
          pdf.cell(0, 10,txt = x16, ln = 1, align = 'L')
      ...
      pdf.cell(0,8,txt='Dehumidifiction Mode:',ln=1,align='L')
      pdf.set_font('Arial', '', 10)
      f9 = open(Config.modes+"heating_mode2.txt", "r")
      for x17 in f9:
    ...
    pdf.set_font('Arial', '', 10)
    f7 = open(Config.all_txt+"COMPRESSORSTIMING.txt", "r")
    for x15 in f7:
    ...
    if Supply=="Supply VFD with PT":
      ...
      pdf.set_font('Arial', '', 10)
      f7a1 = open(Config.all_options+"VFD on supply(PT).txt", "r")
      for x15a1 in f7a1:
      ...
      pdf.set_font('Arial', '', 10)
      f7a1 = open(Config.all_options+"EC.txt", "r")
      for x15a1 in f7a1:
          pdf.multi_cell(0,10,txt=x15a1,border = 0,align='L')
    ...
    if Exhaust=="Exhaust VFD with PT":
      ...
      pdf.set_font('Arial', '', 10)
      f7a1 = open(Config.all_options+"VFD on return(PT).txt", "r")
      for x15a1 in f7a1:
          pdf.multi_cell(0,10,txt=x15a1,border = 0,align='L')
      ...
      pdf.set_font('Arial', '', 10)
      f7a1 = open(Config.all_options+"Exhaust-Return.txt", "r")
      for x15a1 in f7a1:
          pdf.multi_cell(0,10,txt=x15a1,border = 0,align='L')
    else:
      pass
    ...
    if Steam=="Steam_Yes":
      ...
      pdf.set_font('Arial', '', 10)
      f7a1 = open(Config.all_options+"Steam.txt", "r")
      for x15a1 in f7a1:
          pdf.multi_cell(0,10,txt=x15a1,border = 0,align='L')
          # pdf.cell(0, 10,txt = x15, ln = 1, align = 'L')
    ...
    pdf.set_font('Arial', '', 10)
    f7a = open(Config.all_txt+"CONDENSERFANS.txt", "r")
    for x15a in f7a:
    ...
    if Dampers=="Mixing":
      ...
      pdf.set_font('Arial', '', 10)
      f7a1 = open(Config.all_options+"Mixing box.txt", "r")
      for x15a1 in f7a1:
          pdf.multi_cell(0,10,txt=x15a1,border = 0,align='L')
    elif Dampers=="Economizer":
      pdf.set_font('Arial', 'B', 12)
      ...
      pdf.set_font('Arial', '', 10)
      f7a1 = open(Config.all_options+"Economizer.txt", "r")
      for x15a1 in f7a1:
          pdf.multi_cell(0,10,txt=x15a1,border = 0,align='L')
    else:
      pass
    ...
    if options:
      ...
      for e in options:
        ...
        pdf.set_font('Arial', '', 10)
        with open(Config.all_options+e+".txt","r") as f:
            z=f.read()
            # pdf.cell(0,10,txt=z,ln=1,align='L')
            pdf.multi_cell(0,10,txt=z,border = 0,align='L')
        if e=="Extended Keypad(PGD)":
            pdf.image(Config.all_images+"PGD.png",w=90,h=60)
            pdf.image(Config.all_images+"RJ12.png",w=90,h=80)
        if e=="TH Tune":
            pdf.image(Config.all_images+"THTUNE.png",w=60,h=60)
        
        if e=="Extended Keypad":
            pdf.image(Config.all_images+"POL_KEYPAD.PNG",w=90,h=60)
    ...
    pdf.set_font('Arial', '', 10)
    f7a = open(Config.all_txt+"ALARMMANAGEMENT.txt", "r")
    for x15a in f7a:
      pdf.multi_cell(0,10,txt=x15a,border = 0,align='L')
      # pdf.cell(0, 10,txt = x15, ln = 1, align = 'L')
    ...
    #### export pdf file
    pdf.output(Config.SOO_OUTPUT+f"{Project_Name}-{country}.pdf")
    #### convert PDF file to binary to save it in SQL
    time.sleep(2)
    with open(Config.SOO_OUTPUT+f"{Project_Name}-{country}.pdf", "rb") as pdf_file:
  ...
  if  'Send Email' in request.form:
    add_count()
    mail_SOO(username,Config.SOO_OUTPUT+f"{Project_Name}-{country}.pdf")
    return render_template("sequence.html",username=username) 

  elif  'Generate SOO' in request.form:
    add_count()
    ### DOWNLOAD THE FILE
    return send_file(Config.SOO_OUTPUT+f"{Project_Name}-{country}.pdf",as_attachment=True)
